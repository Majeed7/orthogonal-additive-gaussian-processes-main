...
class SHOGP():
    def __init__(self, X, y, inte_order=None, inducing_points=None):
        ...
        if target_type == "continuous":
            ...
            batch_size = np.min((256, X.shape[0]))  # Define the batch size
    ...
    def remove_features_one_by_one(
        ...
        num_remove_feature,          # the number of features to be removed
        ):
        """
        ...
        x_trans = self.OGP._transform_x(x)#.squeeze()  
        
        # Training data (already transformed, presumably)
        X_train_trans = self.X_base
        alpha = self.OGP.alpha.numpy()  # GP's coefficient vector

        # Grab the per-dimension kernel variances
        # (assuming an additive kernel with one sub-kernel per dimension)
        all_sigmas = [var.numpy() for var in self.OGP.m.kernel.variances]
        ...
        active_dim = np.ones((X_train_trans.shape[1],))
        active_dim = np.array(active_dim, dtype=bool)

        for i in range(num_remove_feature): #, dim_to_remove in enumerate(sorted_features):
            # Deactivate the next least important feature
            dim_to_remove = sorted_features[:i+1]
            ...
            active_kernels = [self.OGP.m.kernel.kernels[idx] for idx in active_indices]
            kernel_matrices = [
                k(x_trans, X_train_trans) for k in active_kernels
            ]
            
            additive_terms = self.OGP.m.kernel.compute_additive_terms(kernel_matrices)
            sigmas_active = all_sigmas[:len(additive_terms)]
            ...
            if hasattr(new_pred, 'numpy'):
                new_pred_np = new_pred.numpy()
            else:
                new_pred_np = np.asarray(new_pred)

            # Store the prediction
            predictions_per_step.append(self.OGP.scaler_y.inverse_transform(new_pred_np).item())

        return predictions_per_step
    ...
    def prediction_featuresubset(self, x, active_dim):
        ...
        sigmas = (np.array([self.OGP.m.kernel.variances[i].numpy() for i in range(len(self.OGP.m.kernel.variances))]))
        active_kernels = [self.OGP.m.kernel.kernels[i] for i in active_ind]
        kernel_matrices = [
            k(x_trans[np.newaxis,], self.X_base) for k in active_kernels
        ]  # note that active dims gets applied by each kernel
        additive_terms = self.OGP.m.kernel.compute_additive_terms(kernel_matrices)
        aggregated_kernel = reduce(tf.add, [sigma2 * k for sigma2, k in zip(sigmas, additive_terms)],)

        K = self.OGP.m.kernel(self.X_train)
        Ktilde = K + np.eye(self.X_train.shape[0]) * self.OGP.m.likelihood.variance
        L = np.linalg.cholesky(Ktilde)
        alpha2 = tf.linalg.cholesky_solve(L, self.y_train)

        transformed_pred = aggregated_kernel @ alpha

        return self.OGP.scaler_y.inverse_transform(transformed_pred.numpy().squeeze().reshape(-1,1)) ## Test with the prediction with the model: self.OGP.predict(x[np.newaxis,:])
